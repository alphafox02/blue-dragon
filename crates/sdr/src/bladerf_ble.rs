...
/// BLE channel index (0-39) to center frequency in MHz.
fn ble_channel_to_freq(ch: u8) -> u32 {
    2402 + ch as u32 * 2
}
...
impl BladerfBleHandle {
    ...
    pub fn open(
        ...
        unsafe {
            ...
            // Enable oversample mode for 80 Msps
            if sample_rate > 61_440_000 {
                bladerf_enable_feature(dev, BLADERF_FEATURE_OVERSAMPLE, true);
            }

            // Configure AD9361 (the FPGA uses these settings internally)
            ...
            // The FPGA sends 128-bit words through the same USB bulk endpoint
            // as normal IQ data. libbladeRF treats them as SC16 "samples" --
            // each 128-bit word = 4 complex SC16 "samples" (16 bytes).
            let buf_size: c_uint = 8192;
    ...
    pub fn recv_packets(&mut self) -> Vec<BlePacket> {
        ...
        while pos + WORD_SIZE <= data.len() {
            ...
            //   byte[0..6]  = reserved (56 bits, bytes 0-6)
            //   byte[7]     = aa_errors
            ...
            let freq = ble_channel_to_freq(channel);
            ...
            let pkt = BlePacket {
                ...
                crc_checked: true,
                crc_valid: crc_ok,
                is_data: channel > 36, // channels 37/38/39 are advertising
                conn_valid: false,
                phy,
                ext_header: None,
                data: raw,
